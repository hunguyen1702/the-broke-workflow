...
Use `CLAUDE.md` as the single source of truth for this repository's architecture, CLI surface, workflows, tests, and documentation rules.

Do not duplicate project-specific command lists or architecture notes in `AGENTS.md`; update `CLAUDE.md` instead. This file keeps only agent-runtime instructions that must live in `AGENTS.md`, such as the ByteRover workflow below.
